...
$query = 'select count(*) numero_visualizzazione, id, fk_film, titolo, trama, durata, attori, anno, genere, trailer
          from visualizza, film
          where fk_film = id and data >= \'2020-05-31\' and data < \'2021-05-31\' 
          group by fk_film 
          order by count(*) desc';
...
foreach ($rows as $row) {
    echo     "<tr>
      <th>{$row["id"]}</th>
      <td>{$row["titolo"]}</td>
      <td>{$row["numero_visualizzazione"]}</td>
      <td>{$row["trama"]}</td>
      <td>{$row["genere"]}</td>
      <td>{$row["trailer"]}</td>
      <td>{$row["durata"]}</td>
